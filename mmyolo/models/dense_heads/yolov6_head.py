...
class YOLOV6Head(AnchorFreeHead):

    def loss_by_feat(self, cls_scores: List[Tensor], bbox_preds: List[Tensor], batch_gt_instances: InstanceList,
                     batch_img_metas: List[dict], batch_gt_instances_ignore: OptInstanceList = None) -> dict:
        pass

    def get_targets(self, points: List[Tensor], batch_gt_instances: InstanceList) -> Any:
        pass
    ...
    def __init__(self,
        ...
        self.channels_list = [make_divisible(i * width_multiple, 8) for i in channels_list]
        super().__init__(num_classes, 0, loss_cls=loss_cls, loss_bbox=loss_bbox, bbox_coder=bbox_coder, **kwargs)

        self.na = 1  # anchor-free
        self.no = 5 + self.num_classes
        self.grid = [torch.zeros(1)] * 3
        self.stride = torch.tensor(strides)
        self.loss_fun = ComputeLoss()

    def _init_layers(self):
        head_layers = nn.Sequential(
            # stem0
            vn_layer.Conv(
                in_channels=self.channels_list[6],
                out_channels=self.channels_list[6],
                kernel_size=1,
                stride=1
            ),
            # cls_conv0
            ...
            nn.Conv2d(
                in_channels=self.channels_list[6],
                out_channels=self.num_classes * self.num_base_priors,
                kernel_size=1
            ),
            # reg_pred0
            nn.Conv2d(
                in_channels=self.channels_list[6],
                out_channels=4 * self.num_base_priors,
                kernel_size=1
            ),
            # obj_pred0
            nn.Conv2d(
                in_channels=self.channels_list[6],
                out_channels=1 * self.num_base_priors,
                kernel_size=1
            ),
            # stem1
            vn_layer.Conv(
                in_channels=self.channels_list[8],
                out_channels=self.channels_list[8],
                kernel_size=1,
                stride=1
            ),
            # cls_conv1
            ...
            # stem2
            vn_layer.Conv(
                in_channels=self.channels_list[10],
                out_channels=self.channels_list[10],
                kernel_size=1,
                stride=1
            ),
            # cls_conv2
    ...
    def init_weights(self):
        ...
        for conv in self.cls_preds:
            b = conv.bias.data.view(self.num_base_priors, -1)
    ...
    def loss(self, pred_maps, data_samples):
        ...
        # cls_scores = torch.load('/home/PJLAB/huanghaian/pytorch_code/YOLOv6/tools/aa.pth')
        # outs = ([cls.cuda() for cls in cls_scores],)
        # data_samples = torch.load('/home/PJLAB/huanghaian/pytorch_code/YOLOv6/tools/bb.pth').cuda()

        loss = self.loss_fun(*outs, data_samples)
        return loss

    def predict_by_feat(self, pred_maps, batch_img_metas, rescale=False):
        results = non_max_suppression(pred_maps,
                                      self.test_cfg.conf_thr,
                                      self.test_cfg.nms.iou_threshold,
                                      self.num_classes,
                                      self.test_cfg.agnostic,
                                      self.test_cfg.multi_label,
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            if result.shape[0] == 0:
                ...
                pred_result.scores = result[:, 4]
                pred_result.labels = result[:, 5]
            ...
            pred_result.scores = result[:, 4]
            pred_result.labels = result[:, 5].int()
...
class ComputeLoss:
    ...
    def __call__(
        ...
        for output in outputs:
            assert torch.isfinite(output.sum()), \
                'outputs become infinite or NaN!'
        ...
        return dict(
            ...
            loss_conf=loss_obj * world_size,
            loss_l1=loss_l1 * world_size,
            loss_bbox=loss_iou * self.reg_weight * world_size)
