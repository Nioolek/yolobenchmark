...
def replace_key(key):
    if key.startswith('yolo_head'):
        key = key.replace('yolo_head.', 'bbox_head.')
...
if __name__ == '__main__':
    ...
    if args.flag == 0:
        ...
        with open(args.input_path, 'rb') as f:
            state_dict_paddle = pickle.load(f)
        ...
        for paddle_key in keys_paddle:
            if paddle_key == 'StructuredToParameterName@@':
                continue
            torch_key = replace_key(paddle_key)
            w_paddle = state_dict_paddle[paddle_key]
            print(w_paddle)
            w_torch = torch.tensor(w_paddle, dtype=torch.float)
            state_dict_torch[torch_key] = w_torch
