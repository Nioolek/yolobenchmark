...
def _box_aug_per_img(img, target, aug_type=None, scale_ratios=None, scale_splits=None, img_prob=0.1, box_prob=0.3,
                     level=1):
    ...
    if scale_ratios is None:
        box_sample_prob = [box_prob] * len(target.bboxes)
    else:
        box_sample_prob = [_box_sample_prob(bbox, scale_ratios_splits, box_prob=box_prob) for bbox in target.bboxes]
...
class Box_augs(object):
    def __init__(self, box_augs_dict, max_iters, scale_splits, box_prob=0.3, dynamic_scale_split=False, use_color=True,
                 use_geo=True):
        ...
        self.dynamic_scale_split = dynamic_scale_split
    ...
    def __call__(self, tensor, target, iteration):
        ...
        if self.dynamic_scale_split:
            h, w = tensor.shape[-2:]
            ratio = min(h, w) / 800
        else:
            ratio = 1.0
        scale_splits = [area * ratio for area in self.scale_splits]
        if self.use_color and iter_ratio <= 1:
        ...
        if self.use_geo and iter_ratio <= 1:
