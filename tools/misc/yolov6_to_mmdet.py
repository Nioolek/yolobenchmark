...
def attempt_download(file,
    ...
    if not file.exists():
        ...
        try:
            ...
            assets = [
                'yolov6n.pt', 'yolov6s.pt', 'YOLOv6t.pt'
            ]
            try:
...
def convert(src, dst):
    ...
    for k, v in model.state_dict().items():
        name = k
        if 'detect' in k:
            name = k.replace('detect', 'bbox_head')
...
def main():
    ...
    parser.add_argument('--model', default='yolov6s.pt', help='model name')
    parser.add_argument('--out', default='yolov6s_mm.pt', help='save path')
...
# 该程序必须放在yolov6工程根目录下，读取原始权重后，会自动转换为 mm 支持的模型
##################################################################
