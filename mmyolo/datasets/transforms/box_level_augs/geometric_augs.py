...
def _geometric_aug_func(x, target, angle=0, translate=(0, 0), scale=1, shear=(0, 0), hflip=False, boxes_sample_prob=[],
                        scale_ratio=1.0):
    boxes_and_labels = [(target.bboxes[i], target.labels[i],) for i in range(len(target.bboxes)) if
                        random.random() < boxes_sample_prob[i]]
    ...
    for i, box in enumerate(boxes):
        ...
        if hflip:
            ...
            x_crop = transforms.functional.affine(x_crop, angle, translate, scale, shear, resample=2,
    ...
    if translate[0] + translate[1] != 0 and len(boxes_new) > 0:
        new_target = InstanceData(metainfo={})
        new_target.bboxes = torch.stack(boxes_new)
        new_target.labels = torch.Tensor(labels_new).long()

        target = target.cat([target, new_target])
...
def _transform(x, x_crops, boxes_crops, translate=(0, 0)):
    ...
    for i, box in enumerate(boxes_crops):
        ...
        y[:, x1_c:x1_c + x_crop.shape[1], y1_c:y1_c + x_crop.shape[2]] = g_maps * x_crop + (1 - g_maps) * y_crop[:,
                                                                                                          :x_crop.shape[
                                                                                                              1],
                                                                                                          :x_crop.shape[
                                                                                                              2]]
...
geometric_aug_func = {
    'hflip': lambda x, level, target, boxes_sample_probs: _geometric_aug_func(x, target, hflip=True,
                                                                              boxes_sample_prob=boxes_sample_probs),
    'rotate': lambda x, level, target, boxes_sample_probs: _geometric_aug_func(x, target, level / _MAX_LEVEL * 30,
                                                                               boxes_sample_prob=boxes_sample_probs),
    'shearX': lambda x, level, target, boxes_sample_probs: _geometric_aug_func(x, target,
                                                                               shear=(level / _MAX_LEVEL * 15, 0),
                                                                               boxes_sample_prob=boxes_sample_probs),
    'shearY': lambda x, level, target, boxes_sample_probs: _geometric_aug_func(x, target,
                                                                               shear=(0, level / _MAX_LEVEL * 15),
                                                                               boxes_sample_prob=boxes_sample_probs),
    'translateX': lambda x, level, target, boxes_sample_probs: _geometric_aug_func(x, target, translate=(
    level / _MAX_LEVEL * 120.0, 0), boxes_sample_prob=boxes_sample_probs),
    'translateY': lambda x, level, target, boxes_sample_probs: _geometric_aug_func(x, target, translate=(
    0, level / _MAX_LEVEL * 120.0), boxes_sample_prob=boxes_sample_probs)}
